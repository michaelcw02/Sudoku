...
var OptionsComponent = (function () {
    ...
    OptionsComponent.prototype.solveOptions = function () {
        var selected = __WEBPACK_IMPORTED_MODULE_4_jquery__('#solveBySel option:selected').val();
        switch (selected) {
            case "BACKTRACK":
                this.communicationService.callSolve();
                break;
            case "BACKTRACK STEP BY STEP":
            ...
            case "SOLVE BY...":
                ...
                __WEBPACK_IMPORTED_MODULE_4_jquery__("#messageError").text("Please, select a valid option to solve the game.");
                break;
            default:
                this.modalRef = this.modalService.show(this.errorModal);
                __WEBPACK_IMPORTED_MODULE_4_jquery__("#messageError").text("Please, select a valid option to solve the game.");
...
var SudokuComponent = (function () {
    ...
    SudokuComponent.prototype.compareGrids = function (oldGrid, newGrid) {
        var result = true;
        oldGrid.forEach(function (x, i) { return x.forEach(function (y, j) {
            if (y != newGrid[i][j].value)
                result = false;
        }); });
        if (result)
    ...
    SudokuComponent.prototype.solveByNakedSingle = function () {
        var _this = this;
        var oldGrid = this.sudokuHelper.gridToMatrix(this.sudoku.grid);
        var interval = setInterval(function () {
            if (_this.nakedSingleSolver.solve(_this.sudoku)) {
                clearInterval(interval);
                _this.compareGrids(oldGrid, _this.sudoku.grid);
            }
        }, 1000);
    };
    SudokuComponent.prototype.solveByHiddenSingle = function () {
        var _this = this;
        var oldGrid = this.sudokuHelper.gridToMatrix(this.sudoku);
        var interval = setInterval(function () {
    ...
    SudokuComponent.prototype.saveSudoku = function (user) {
        ...
        this.sudokuService.saveSudoku(user, this.sudoku, function (err, res) {
            err ? _this.showError("There was an error saving the match.")
                : _this.gameSaved(user);
        });
    };
    SudokuComponent.prototype.gameSaved = function (user) {
